...
namespace TxtOper
{
    public partial class FiltrateFile : Form
    {
        ...
        private void btcz_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                pGetAllDirFiles(path, tBsp.Text, 1000, ref files, ref paths, ref lv);
            }
            catch (Exception ex)
            {
                MessShow(ex.Message);
        ...
        {
            int cgyes = 0; 
            string outxx = "[" + czxh.ToString() + "] >>> 参数读取完毕，共计搜索到[" + files.Count + "]个需要操作的文件。操作开始！ 操作时间： " + DateTime.Now.ToString() + "\r\n";
            savehs.Append(outxx);
            OT(outxx);
            for (int i = 0; i < files.Count;i++ ) 
            {
                string fstr = files[i].ToString().Trim();
                if (fstr!="")
                {
                    try
                    {
                        ...
                        OT("[" + czxh.ToString() + "] >>> 操作文件路径：[" + fstr + "]、文件名：[" + fname + "]、字节数：[" + lstr.Length.ToString() + "]，保存操作开始。操作时间： " + DateTime.Now.ToString() + "\r\n");
                        File.Delete(fstr);
                        string newpath = fpath + "\\" + fileName;
                        FileStream fc = File.Create(newpath);
                        byte[] bContent = Encoding.GetEncoding(ens).GetBytes(lstrv);
                        fc.Write(bContent, 0, bContent.Length);
                        fc.Close();
                        OT("[" + czxh.ToString() + "] >>> 保存文件路径：[" + newpath + "]、文件名：[" + fileName + "]、字节数：[" + lstrv.Length.ToString() + "]，保存操作成功。操作时间： " + DateTime.Now.ToString() + "\r\n");
                        cgyes++;
                    }
                    catch (Exception e)
                    {
            ...
            outxx = "[" + czxh.ToString() + "] >>> 操作完毕，共计操作成功[" + cgyes.ToString() + "]个文件。 操作时间： " + DateTime.Now.ToString() + "\r\n";
            savehs.Append(outxx);
            OT(outxx);
        }
